...
public class TestMainActivity extends AppCompatActivity implements View.OnClickListener, CompoundButton.OnCheckedChangeListener, FingerListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imageView.setImageDrawable(getDrawable(R.drawable.idbg));
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_USB_PERMISSION.equals(action))
            {
                synchronized (this)
                {
                    ...
                    {
                        OpenDevice();
                        result = fingerHelper.open(getContext());
                    }
                    else
                    {
                        Toast.makeText(mContext, "USB未授权", Toast.LENGTH_SHORT).show();
                        ...
                        if (retType == 1 || retType == 2 || retType == 3)
                        {
                            ...
                            runOnUiThread(new Runnable() {
                                public void run() {
                                    ...
                                    if (final_retType == 1 && idCardInfo != null)
                                    {
                                        //final IDCardInfo idCardInfo = idCardReader.getLastIDCardInfo();
                                        //姓名adb
